...
function update(time){
  ...
  obstacleSpeed+=.00005;
    //moving rail tracks
  moveRailTracks(obstacleSpeed, deltaTime);
  obstacles.forEach(({element,trackNo,name},index)=>{
    ...
    if(trackNo==playerTrack){
      detectCollision(obsTop,obsHeight,name)
    }
  });
  //moving coins
  if(coinChainMove){
...
function detectCollision(top,height,name){
  ...
  if(top>minTop && top<maxTop){
    if((name=="slidable" && !isSliding) || (name=="jumpable" && !isJumping) || name=="train"){
      console.log(name,isSliding);
      gameOver();
    }
  }
}
function moveRailTracks(obstacleSpeed,deltaTime){
  document.querySelectorAll(".track").forEach(track=>{
    let line1=track.querySelector(".line-1");
    let line2=track.querySelector(".line-2");
    let line1Top=parseFloat(getComputedStyle(line1).getPropertyValue("top"));
    let line2Top=parseFloat(getComputedStyle(line2).getPropertyValue("top"));
    if(line1Top>=gameHeight){
      line1.style.top=-gameHeight+"px";
      return;
    }
    if(line2Top>=gameHeight){
      line2.style.top=-gameHeight+"px";
      return;
    }
    line1Top+=obstacleSpeed*deltaTime;
    line1.style.top=line1Top+"px";
    line2Top+=obstacleSpeed*deltaTime;
    line2.style.top=line2Top+"px";
  })
